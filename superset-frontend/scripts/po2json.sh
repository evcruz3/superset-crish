...
do
  ...
  if [ $extension == "po" ]
  then
    echo "npx po2json --domain superset --format jed1.x $file $filename.json"
    npx po2json --domain superset --format jed1.x $file $filename.json
    npx prettier --write $filename.json
  fi
done
