...
const CreateWhatsAppGroupModal: React.FC<CreateWhatsAppGroupModalProps> = ({ visible, onClose, onSuccess, addSuccessToast }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      setIsLoading(false);
      if (response.json.id) {
        addSuccessToast(t('Successfully created WhatsApp group: %s', values.name));
        onSuccess();
        form.resetFields();
      } else {
        throw new Error(response.json.message || 'Failed to create WhatsApp group');
      }
    } catch (error: any) {
      setIsLoading(false);
      let errorMessage = t('An error occurred while creating the WhatsApp group.');
          ...
          rules={[
            ...
            // More robust phone number validation (e.g., E.164 format) could be added here.
            {
              validator: (_, value) => {
                ...
                if (invalidNumbers.length > 0 && numbersArray.length === invalidNumbers.length) { // if all entries after split are empty
