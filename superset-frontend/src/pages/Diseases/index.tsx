...
const getMetricColor = (
  ...
  const level = METRIC_LEVELS.find(
...
function Diseases() {
  ...
  const [selectedMetric, setSelectedMetric] =
    useState<DiseaseMetric>('totalCases');
    ...
    const fetchGeojson = async () => {
      try {
        ...
        if (!geojsonResponse.ok) {
          ...
          throw new Error(
            `Failed to load GeoJSON map data. Status: ${geojsonResponse.status}.`,
          );
        }
      ...
      try {
        ...
        if (selectedDisease === 'Diarrhea') {
          ...
          const results = responses.map(
            res => res.json.result as DiseaseData[],
          );
          ...
          results.flat().forEach(record => {
            ...
            if (existing) {
              ...
              dataMap.set(record.municipality_code, {
                ...record,
                disease: 'Diarrhea',
              });
            }
  ...
  const geoJsonLayer =
    geojson &&
    new GeoJsonLayer<JsonObject, GeoJsonLayerProps<JsonObject>>({
      id: 'geojson-layer',
      data: geojson as any,
      pickable: true,
      stroked: true,
      ...
      getFillColor: (d: any) => {
        ...
        const record = mapData.find(
          f => f.municipality_code === municipalityCode,
        );
  ...
  return (
    <StyledTabsContainer theme={theme}>
      <LineEditableTabs
        ...
        type="card"
      >
        <LineEditableTabs.TabPane tab="Overview" key="overview">
        ...
        <LineEditableTabs.TabPane tab="Death Cases" key="death_cases">
          <TabContentContainer>
            <ChartContainer>
              <ResponsiveChartSlug
