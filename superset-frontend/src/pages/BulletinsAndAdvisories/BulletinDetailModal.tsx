...
const StyledModal = styled(Modal)`
  ...
  .ant-carousel {
    ...
    .slick-next {
      font-size: ${({ theme }) =>
        theme.typography.sizes.xl}px; // Increase icon size
    ...
    .slick-dots li button {
      background: ${({ theme }) =>
        theme.colors.primary.base}; // Make dots more visible too
    }
...
export default function BulletinDetailModal({
  ...
  const [updatedBulletin, setUpdatedBulletin] = useState<Bulletin | null>(
    bulletin,
  );
      ...
      >
        ...
        <div className="bulletin-meta">
          {updatedBulletin.created_by && (
            <>
              {t('Posted by')}{' '}
              {`${updatedBulletin.created_by.first_name} ${updatedBulletin.created_by.last_name}`}{' '}
            </>
          )}
          ...
          {updatedBulletin.changed_on &&
            updatedBulletin.changed_on !== updatedBulletin.created_on && (
              <>
                • {t('Last updated')}{' '}
                {moment(updatedBulletin.changed_on).fromNow()}
              </>
            )}
        </div>

        {/* Image Attachments Section */}
        {updatedBulletin?.image_attachments &&
          updatedBulletin.image_attachments.length > 0 && (
            <div className="bulletin-section">
              ...
              {updatedBulletin.image_attachments.length === 1 ? (
                ...
                <Carousel autoplay dotPosition="top">
                  {updatedBulletin.image_attachments.map(
