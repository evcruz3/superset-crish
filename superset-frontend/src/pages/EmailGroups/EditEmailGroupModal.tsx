...
const EditEmailGroupModal: React.FC<EditEmailGroupModalProps> = ({ emailGroup, visible, onClose, onSuccess, addSuccessToast }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.json && (response.json.id || (response.json.result && response.json.result.id))) {
        ...
        throw new Error(response.json.message || 'Failed to update email group');
      }
    } catch (error: any) {
      setIsLoading(false);
    ...
    >
      <Form form={form} layout="vertical" name="editEmailGroupForm">
        ...
        <Form.Item
          ...
          rules={[
            { required: true, message: t('Please input at least one email address!') },
            {
              validator: (_, value) => {
                if (!value) return Promise.resolve();
                const emailsArray = value.split(',').map((e: string) => e.trim());
                const invalidEmails = emailsArray.filter((e: string) => !/^[\w.-]+@[\w.-]+\.\w+$/.test(e));
                if (invalidEmails.length > 0) {
                  return Promise.reject(new Error(t('Invalid email format found: %s', invalidEmails.join(', '))));
                }
        ...
        >
          <Input.TextArea rows={3} placeholder={t('user1@example.com, user2@example.com')} />
        </Form.Item>
      </Form>
    </Modal>
  );
