...
def upgrade():
    ...
    # Create disease_forecast_alerts table first (always create, use CREATE TABLE IF NOT EXISTS)
    conn = op.get_bind()
...
def downgrade():
    ...
    conn = op.get_bind()
    conn.execute(sa.text("DROP TABLE IF EXISTS disease_forecast_alerts"))
    # ### end Alembic commands ###
