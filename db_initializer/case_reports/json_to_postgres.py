...
def create_table(conn):
    ...
    with conn.cursor() as cur:
        ...
        # Create a unique constraint on the combination of fields
        cur.execute(f'''
        DO $$
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint WHERE conname = '{TABLE_NAME}_unique_report'
            ) THEN
                ALTER TABLE {TABLE_NAME}
                ADD CONSTRAINT {TABLE_NAME}_unique_report 
